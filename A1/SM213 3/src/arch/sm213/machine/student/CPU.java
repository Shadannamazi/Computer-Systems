  ...
  @Override protected void execute () throws InvalidInstructionException, MachineHaltException, RegisterSet.InvalidRegisterNumberException, MainMemory.InvalidAddressException
  {
    switch (insOpCode.get()) {
      ...
      case 0x1: // ld o(rs), rd .......... 1psd  (p = o / 4)
        // TODO
          ; //s
          int o = insOp0.get() * 4; //p*4 = o

          int r_s = reg.get(insOp1.get()); //r[s]
          int sum_address = o + r_s;

          reg.set(insOp2.get(), mem.readInteger(sum_address));
        break;
      case 0x2: // ld (rs, ri, 4), rd .... 2sid
        int r_s2 = reg.get(insOp0.get()); //r[s]
        int r_i_4 = reg.get(insOp1.get()) * 4; //r[i] * 4
          reg.set(insOp2.get(),mem.readInteger(r_s2 + r_i_4));

        break;
      case 0x3: // st rs, o(rd) .......... 3spd  (p = o / 4)
        int o_1 = insOp1.get() * 4; //o
        int d = reg.get(insOp2.get()); //r[d]
        //int mem_sum =  mem.readInteger(d+o_1);

          mem.writeInteger(d+o_1,reg.get(insOp0.get()));
        break;
      case 0x4: // st rs, (rd, ri, 4) .... 4sdi
        int r_d_2 = reg.get(insOp1.get()); //r[d]
        int r_i_times4 = reg.get(insOp2.get()) * 4;
        mem.writeInteger(r_d_2 + r_i_times4,reg.get(insOp0.get()));
        break;
      case 0x6: // ALU ................... 6-sd
	switch (insOp0.get()) {
